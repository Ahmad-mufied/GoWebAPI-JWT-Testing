...
func Test_app_userHandlers(t *testing.T) {
	var tests = []struct {
		...
		{"getUser bad URL param", "GET", "", "y", app.getUser, http.StatusBadRequest},
		{
			"updateUser valid",
			"PATCH",
			`{"id":1,"first_name":"Administartor","last_name":"User","email":"admin@example.com"}`,
			"",
			app.updateUser,
		...
		},
		{
			"updateUser invalid",
			"PATCH",
			`{"id":100,"first_name":"Administartor","last_name":"User","email":"admin@example.com"}`,
			"",
			app.updateUser,
...
func Test_app_refreshUsingCookie(t *testing.T) {
	...
	for _, e := range tests {
		...
		if rr.Code != e.expectStatus {
			t.Errorf("%s: wrong status code returned; expected %d but got %d", e.name, e.expectStatus, rr.Code)
		}
	}
}
