...
func Test_app_getTokenFromHeaderAndVerify(t *testing.T) {
	...
	var tests = []struct {
		...
		{"invalid expired", fmt.Sprintf("Bearer %s1", tokens.Token), true, true, app.Domain},
		...
		// make sure teh next test is the last one to run
		{"wrong issuer", fmt.Sprintf("Bear %s 1", tokens.Token), true, true, app.Domain}}
